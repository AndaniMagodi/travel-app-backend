...
const carRentalProvider = require('./providers/carRental');
...
// One endpoint that runs all four providers in parallel and returns
...
app.get('/api/trip', async (req, res) => {
  ...
  const tasks = {
    ...
    hotels: toSafeResult(getResults(city, { checkIn, checkOut }, hotelProvider)),
    flights: toSafeResult(getResults(city, { fromCity, departDate: checkIn }, flightProvider)),
    carRental: toSafeResult(getResults(city, { pickupDate: checkIn, dropoffDate: checkOut }, carRentalProvider)),
  };
  ...
  SearchHistory.create({ city, fromCity, checkIn, checkOut, results })
